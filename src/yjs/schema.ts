 ...
 * 
 * Verified Fix: Uses y-prosemirror to correctly deserialize Yjs state to JSON
 */
export function getAllBlocks(doc: Y.Doc): Block[] {
    ...
    // Priority 2: Legacy Content Array
...
export function getAllBlocks(doc: Y.Doc): Block[] {
    ...
    if (fragment && fragment.length > 0) {
        // Use official helper to convert to proseMirror JSON
        // This handles all merging of text nodes, marks, etc.
        const serialized = yXmlFragmentToProsemirrorJSON(fragment);

        // serialized is an Array of JSON blocks (if fragment is a list of nodes)
        // or an Object if it mimics a Doc.
        // yXmlFragmentToProsemirrorJSON returns an ARRAY of nodes [node, node]
        ...
        if (serialized && Array.isArray(serialized)) {
            for (const node of serialized) {
                ...
                // Extract text from PM JSON content (which is an array of text nodes)
                let text = '';
                if (node.content && Array.isArray(node.content)) {
                    text = node.content
                        .map((c: any) => c.text || '')
