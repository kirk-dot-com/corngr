        ...
        match entry.get_password() {
            Ok(secret_hex) => {
                ...
                let bytes = hex::decode(secret_hex)
                    .map_err(|e| KeyStoreError::StorageError(format!("Invalid key format: {}", e)))?;
                
                let bytes_sized: [u8; 32] = bytes.try_into()
...
impl KeyStore for KeychainStore {
    fn get_public_key(&self) -> Result<VerifyingKey, KeyStoreError> {
        ...
        let bytes = hex::decode(secret_hex).map_err(|e| KeyStoreError::StorageError(e.to_string()))?;
        let bytes_sized: [u8; 32] = bytes.try_into().map_err(|_| KeyStoreError::StorageError("Invalid key length".into()))?;
        let key = SigningKey::from_bytes(&bytes_sized);
    ...
    fn sign(&self, message: &[u8]) -> Result<Signature, KeyStoreError> {
        let entry = self.get_entry()?;
        let secret_hex = entry.get_password().map_err(|e| KeyStoreError::StorageError(e.to_string()))?;
         let bytes = hex::decode(secret_hex).map_err(|e| KeyStoreError::StorageError(e.to_string()))?;
        let bytes_sized: [u8; 32] = bytes.try_into().map_err(|_| KeyStoreError::StorageError("Invalid key length".into()))?;
        let key = SigningKey::from_bytes(&bytes_sized);
    ...
    fn rotate_key(&mut self) -> Result<VerifyingKey, KeyStoreError> {
        ...
        let entry = self.get_entry()?;
        entry.set_password(&hex_str).map_err(|e| KeyStoreError::StorageError(e.to_string()))?;
